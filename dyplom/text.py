...
class Text:
    def __init__(self, name):
        self.name = name
        if os.path.isfile(self.name + '\\chapters.txt'):
            self.chapters = []
            with open(self.name + '\\chapters.txt', 'r',encoding='utf-8')as f:
                line = f.readline()
                line = line.split('\t')
                dict_ch = {'start_time':line[0],'title':line[1], 'end_time':line[2] }
                self.chapters.append(dict_ch)
            timing_dict = {}
    ...
    def find_chapter_for_text(self, timing_dict):
        ...
        for start_time, (text, end_time) in timing_dict.items():
            ...
            for index, chapter in enumerate(self.chapters):
                if end_time <= float(chapter['end_time']) and start_time >= float(chapter['start_time']):
                    title = chapter['title']
